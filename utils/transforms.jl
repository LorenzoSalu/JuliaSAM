...
function apply_image(
	...
	resized_image = to_pil_image(image)
	
	#is_correct("to_pil_image corretto? ", resized_image, expected_to_pil_image)
	
	final_image = torchvision_resize_exact(resized_image, target_size)
...
function apply_coords(
	...
	new_h, new_w = get_preprocess_shape(
		original_size[1], original_size[2], self.target_length)

	#is_correct("first coords transformer corretto? ",coords, expected_transformer_first_coords)
	...
	#is_correct("second coords transformer corretto? ",coords, expected_transformer_second_coords)

	coords[colons..., 2] .= coords[colons..., 2] .* (new_h / old_h)

	#is_correct("third coords transformer corretto? ",coords, expected_transformer_third_coords)
